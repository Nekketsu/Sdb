...
TEST_CASE("Reading and wriing memory works", "[memory]")
{
    bool close_on_exec = false;
    sdb::pipe channel(close_on_exec);
...
TEST_CASE("Find main", "[dwarf]")
{
    ...
    for (auto& cu : dwarf.compile_units())
    {
        ...
        {
            if (die.abbrev_entry()->tag == DW_TAG_subprogram and die.contains(DW_AT_name))
            {
...
TEST_CASE("Can read global integer variable", "[variable]")
{
    ...
    auto var_loc = var_die.value()[DW_AT_location].as_evaluated_location(proc, proc.get_registers(), false);
...
TEST_CASE("Global variables", "[variable]")
{
    ...
    auto name = target->resolve_indirect_name("sy.pets[0].name", target->get_pc_file_address());
...
TEST_CASE("Member pointers", "[variable]")
{
    ...
    auto data_ptr = target->resolve_indirect_name("data_ptr", target->get_pc_file_address());
