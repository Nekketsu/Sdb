...
namespace sdb
{
    ...
    public:
        ...
        type strip_cv_ref_typedef() const
        {
            return strip<DW_TAG_const_type,
                         DW_TAG_volatile_type,
                         DW_TAG_typedef,
    ...
    public:
            ...
            typed_data fixup_bitfield(const sdb::process& proc, const sdb::die& member_die) const;
            std::string visualize(const sdb::process& proc, int depth = 0) const;

            typed_data defer_pointer(const sdb::process& proc) const;
            typed_data read_member(const sdb::process& proc, std::string_view member_name) const;
