...
namespace
{
    ...
    sdb::line_table::file parse_line_table_file(cursor& cur, std::filesystem::path compilation_dir, const std::vector<std::filesystem::path>& include_directories)
    {
        ...
        std::uint64_t data_section_start, std::uint64_t func_start)
    {
        ...
        case DW_EH_PE_pcrel: base = pc; break;
        case DW_EH_PE_textrel: base = text_section_start; break;
        case DW_EH_PE_datarel: base = data_section_start; break;
        case DW_EH_PE_funcrel: base = func_start; break;
        default: sdb::error::send("Unknown eh_frame pointer encoding");
        }
    ...
    sdb::call_frame_information::common_information_entry parse_cie(cursor cur)
    {
        ...
        return
        {
            length, code_alignment_factor, data_alignment_factor, fde_has_augmentation, fde_pointer_encoding, instructions
        };
    }

    sdb::call_frame_information::frame_description_entry parse_fde(const sdb::call_frame_information& cfi, cursor cur)
    {
        auto start = cur.position();
        auto length = cur.u32() + 4;
        ...
        auto text_section_start = elf->get_section_start_address(".text").value_or(sdb::file_addr{});
    ...
    std::unique_ptr<sdb::call_frame_information> parse_call_frame_information(sdb::dwarf& dwarf)
    {
    ...
    struct unwind_context
    {
        ...
        using rule = std::variant<undefined_rule, same_rule, offset_rule, val_offset_rule, register_rule, expr_rule, val_expr_rule>;
        ...
        unwind_context& ctx, sdb::file_addr pc)
    {
        ...
        auto text_section_start = *elf.get_section_start_address(".text");
        auto plt_start = elf.get_section_start_address(".got.plt").value_or(sdb::file_addr{});
    ...
    sdb::registers execute_unwind_rules(unwind_context& ctx, sdb::registers& old_regs, const sdb::process& proc)
    {
        ...
        for (auto [reg, rule] : ctx.register_rules)
        {
            ...
            else if (auto offset = std::get_if<offset_rule>(&rule))
            {
                ...
                auto value = sdb::from_bytes<std::uint64_t>(proc.read_memory(addr, 8).data());
    ...
    sdb::virt_addr read_frame_base_result(const sdb::dwarf_expression::result& loc, const sdb::registers& regs)
    {
    ...
    void scopes_at_address_in_die(const sdb::die& die, sdb::file_addr address, std::vector<sdb::die>& scopes)
    {
...
sdb::range_list::iterator sdb::range_list::end() const
{
    return {};
}
    ...
    if (func)
    {
        ...
        while (true)
        {
            ...
            auto found = std::find_if(children.begin(), children.end(),
                [=](auto& child)
                {
                    return child.abbrev_entry()->tag == DW_TAG_inlined_subroutine and child.contains_address(address);
                });
...
const sdb::call_frame_information::common_information_entry& sdb::call_frame_information::get_cie(file_offset at) const
{
    auto offset = at.off();
...
const std::byte* sdb::call_frame_information::eh_hdr::operator[](file_addr address) const
{
    ...
    while (low <= high)
    {
        ...
        auto entry_address = parse_eh_frame_pointer(*elf, cur, encoding, current_offset.off(), text_section_start.addr(), eh_hdr_offset.off(), 0);
    ...
    auto current_offset = elf->data_pointer_as_file_offset(cur.position());
    auto eh_hdr_offset = elf->data_pointer_as_file_offset(start);
    auto fde_offset_int = parse_eh_frame_pointer(*elf, cur, encoding, current_offset.off(), text_section_start.addr(), eh_hdr_offset.off(), 0);
    sdb::file_offset fde_offset{ *elf, fde_offset_int };
    return elf->file_offset_as_data_pointer(fde_offset);
}

sdb::registers sdb::call_frame_information::unwind(const sdb::process& proc, file_addr pc, registers& regs) const
{
    ...
    if (pc < fde.initial_location or pc >= fde.initial_location + fde.address_range)
    {
...
sdb::dwarf_expression::result sdb::dwarf_expression::eval(const sdb::process& proc, const registers& regs, bool push_cfa) const
{
    ...
    while (!cur.finished())
    {
        ...
        case DW_OP_bregx:
        {
            auto reg_val = regs.read(sdb::register_info_by_dwarf(cur.uleb128()));
        ...
        {
            auto offset = cur.sleb128();
            auto fb_loc = func.value()[DW_AT_frame_base].as_evaluated_location(proc, regs, /*in_frame_info=*/true);
        ...
        {
            auto length = cur.uleb128();
            most_recent_location = data_result{ span<const std::byte>{ cur.position(), length } };
            break;
        }
...
sdb::dwarf_expression::result sdb::location_list::eval(const sdb::process& proc, const registers& regs) const
{
    auto virt_pc = virt_addr{ regs.read_by_id_as<std::uint64_t>(register_id::rip) };
    auto pc = virt_pc.to_file_addr(*parent_->elf_file());
    auto func = parent_->function_containing_address(pc);
...
{
    auto section = cu_->dwarf_info()->elf_file()->get_section_contents(".debug_loc");
...
std::optional<sdb::die::bitfield_information> sdb::die::get_bitfield_information(std::uint64_t class_byte_size) const
{
