...
const sdb::elf* sdb::elf_collection::get_elf_containing_address(virt_addr address) const
{
    for (auto& elf : elves_)
    {
...
const sdb::elf* sdb::elf_collection::get_elf_by_path(std::filesystem::path path) const
{
    for (auto& elf : elves_)
    {
        if (elf->path() == path)
