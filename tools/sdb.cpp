...
namespace
{
    ...
    void print_stop_reason(const sdb::target& target, sdb::stop_reason reason)
    {
        ...
        case sdb::process_state::exited:
            fmt::format("Process {} exited with status {}",
                target.get_process().pid(),
                static_cast<int>(reason.info));
            return;
        case sdb::process_state::terminated:
            fmt::format("Process {} terminated with signal {}",
                target.get_process().pid(),
    ...
    void handle_breakpoint_set_command(sdb::target& target, const std::vector<std::string>& args)
    {
        bool hardware = false;
        if (args.size() == 4)
        {
    ...
    void handle_breakpoint_toggle(sdb::target& target, const std::vector<std::string>& args)
    {
        auto command = args[1];
    ...
    void handle_breakpoint_command(sdb::target& target, const std::vector<std::string>& args)
    {
        if (args.size() < 2)
        {
        ...
        for (auto& scope : scopes)
        {
            for (auto& var : scope.children())
            {
                    ...
                    !name.empty() and !seen.count(name))
                {
                    ...
                    auto str = sdb::typed_data{ std::move(value), type }.visualize(target.get_process());
                    fmt::print("{}: {}\n", name, str);
                    seen.insert(name);
                }
            }
    ...
    void handle_variable_read_command(sdb::target& target, const std::vector<std::string>& args)
    {
        auto name = args[2];
        auto pc = target.get_pc_file_address();
    ...
    void handle_variable_location_command(sdb::target& target, const std::vector<std::string>& args)
    {
        auto name = args[2];
        auto pc = target.get_pc_file_address();
        ...
        else if (auto pieces_res = std::get_if<sdb::dwarf_expression::pieces_result>(&loc))
        {
            ...
            {
                fmt::print("Piece: offset = {}, bit size = {}, location = ", piece.offset, piece.bit_size);
                print_simple_location(&piece.location);
            }
        }
    }

    void handle_variable_command(sdb::target& target, const std::vector<std::string>& args)
    {
        if (args.size() < 2)
        {
