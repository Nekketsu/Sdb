...
namespace
{
    ...
    {
        return fmt::format("0x{:x}", sdb::from_bytes<std::uintptr_t>(data.data_ptr()));
    }

    std::string visualize_pointer_type(const sdb::process& proc, const sdb::typed_data& data)
    {
    ...
    std::string visualize_class_type(const sdb::process& proc, const sdb::typed_data& data, int depth)
    {
        std::string ret = "{\n";
    ...
    std::string visualize_subrange(const sdb::process& proc, const sdb::type& value_type, sdb::span<const std::byte> data, std::vector<std::size_t> dimensions)
    {
        if (dimensions.empty())
        {
...
std::size_t sdb::type::compute_byte_size() const
{
    ...
    auto& die = std::get<sdb::die>(info_);
    ...
    {
        auto member_type = die[DW_AT_type].as_type();
    ...
    {
        auto value_size = die[DW_AT_type].as_type().byte_size();
    ...
    if (die.contains(DW_AT_byte_size))
    {
        return die[DW_AT_byte_size].as_int();
    }
    if (die.contains(DW_AT_type))
    {
        return die[DW_AT_type].as_type().byte_size();
    }
...
std::string sdb::typed_data::visualize(const sdb::process& proc, int depth) const
{
    ...
    case DW_TAG_volatile_type:
        return sdb::typed_data{ data_, die[DW_AT_type].as_type() }.visualize(proc);
...
{
    std::array<parameter_class, 2> classes = { parameter_class::no_class, parameter_class::no_class };
...
bool sdb::type::has_unaligned_fields() const
{
    ...
    if (is_class_type())
    {
        for (auto child : get_die().children())
        {
            ...
            {
                auto member_type = child[DW_AT_type].as_type();
                if (child[DW_AT_data_member_location].as_int() % member_type.alignment() != 0)
                {
                    return true;
                }
