...
namespace
{
    ...
    sdb::typed_data get_initial_variable_data(const sdb::target& target, std::string name, sdb::file_addr pc)
    {
        ...
        auto loc = var.value()[DW_AT_location].as_evaluated_location(target.get_process(), target.get_stack().current_frame().regs, false);
    ...
    std::vector<sdb::typed_data> collect_arguments(sdb::target& target, pid_t tid, std::string_view arg_string, const std::vector<sdb::die>& funcs, std::optional<sdb::typed_data> object)
    {
...
sdb::target::find_functions_result sdb::target::find_functions(std::string name) const
{
...
sdb::breakpoint& sdb::target::create_address_breakpoint(virt_addr address, bool hardware, bool internal)
{
    return breakpoints_.push(std::unique_ptr<address_breakpoint>(new address_breakpoint(*this, address, hardware, internal)));
}

sdb::breakpoint& sdb::target::create_function_breakpoint(std::string function_name, bool hardware, bool internal)
{
    return breakpoints_.push(std::unique_ptr<function_breakpoint>(new function_breakpoint(*this, function_name, hardware, internal)));
}

sdb::breakpoint& sdb::target::create_line_breakpoint(std::filesystem::path file, std::size_t line, bool hardware, bool internal)
{
    return breakpoints_.push(std::unique_ptr<line_breakpoint>(new line_breakpoint(*this, file, line, hardware, internal)));
}
...
std::vector<std::byte> sdb::target::read_location_data(const dwarf_expression::result& loc, std::size_t size, std::optional<pid_t> otid) const
{
    auto tid = otid.value_or(process_->current_thread());
    ...
    if (auto simple_loc = std::get_if<sdb::dwarf_expression::simple_location>(&loc))
    {
        ...
        else if (auto addr_res = std::get_if<sdb::dwarf_expression::address_result>(simple_loc))
        {
        ...
        else if (auto literal_res = std::get_if<sdb::dwarf_expression::literal_result>(simple_loc))
        {
...
std::optional<sdb::die> sdb::target::find_variable(std::string name, sdb::file_addr pc) const
{
    auto& dwarf = pc.elf_file()->get_dwarf();
...
sdb::target::resolve_indirect_name_result sdb::target::resolve_indirect_name(std::string name, sdb::file_addr pc) const
{
...
sdb::virt_addr sdb::target::inferior_malloc(std::size_t size)
{
    ...
    auto malloc_func = std::find_if(malloc_funcs.begin(), malloc_funcs.end(), [](auto& sym)
    {
