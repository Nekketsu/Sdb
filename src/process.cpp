...
namespace
{
    ...
    void set_ptrace_options(pid_t pid)
    {
        ...
        {
            sdb::error::send_errno("Failed to set TRACESYSGOOD and TRACECLONE options");
        }
    }
}
...
std::unique_ptr<sdb::process> sdb::process::attach(pid_t pid)
{
    ...
    bool attached = true;
    std::unique_ptr<process> proc (new process(pid, terminate_on_end, attached));
    proc->wait_on_signal();
    set_ptrace_options(proc->pid());
...
void sdb::process::write_user_area(std::size_t offset, std::uint64_t data, std::optional<pid_t> otid)
{
...
void sdb::process::write_fprs(const user_fpregs_struct& fprs, std::optional<pid_t> otid)
{
    auto tid = otid.value_or(current_thread_);
...
std::vector<std::byte> sdb::process::read_memory(virt_addr address, std::size_t amount) const
{
    ...
    {
        error::send_errno("Cound not read process memory");
    }
    return ret;
}

std::vector<std::byte> sdb::process::read_memory_without_traps(virt_addr address, size_t amount) const
{
...
std::variant<sdb::breakpoint_site::id_type, sdb::watchpoint::id_type> sdb::process::get_current_hardware_stoppoint(std::optional<pid_t> otid) const
{
...
const sdb::registers& sdb::process::get_registers(std::optional<pid_t> otid) const
{
...
void sdb::process::report_thread_lifecycle_event(const sdb::stop_reason& reason)
{
